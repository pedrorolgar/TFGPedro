...
public class PlatosController {
  ...
  // Cargar los platos desde la base de datos y retornar el panel con los datos
  public JScrollPane cargarPlatos(JScrollPane menuPlatos, String nifUsuario) {
    String query = "SELECT * FROM producto WHERE categoria = 'Plato'";
    try {
      Statement stmt = connection.createStatement();
      ResultSet rs = stmt.executeQuery(query);

      // Crear el panel contenedor para los platos
      JPanel panelContenedor = new JPanel();
      panelContenedor.setLayout(new GridLayout(0, 3, 10, 10));

      // Crear un panel para cada plato
      while (rs.next()) {
        ...
        JPanel panel = crearPanelPlato(nombre, descripcion, precioUnitario, idProducto, nifUsuario);
      ...
      menuPlatos.setViewportView(panelContenedor);
      menuPlatos.revalidate();
      menuPlatos.repaint();
    } catch (SQLException e) {
      e.printStackTrace();
  ...
  // Crear el panel para cada plato
  private JPanel crearPanelPlato(String nombre, String descripcion, double precioUnitario, int idProducto, String nifUsuario) {
